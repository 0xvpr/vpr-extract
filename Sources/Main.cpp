...
using io_t = std::pair<std::string, std::string>;
...
int main(int argc, char** argv) {
    ...
    --argc;
    ++argv;
    std::vector<io_t> targets((argc) & 0xFF);
    for (int i = 0; i < argc; ++i) {
        std::cout << argv[i] << " " << "out.bin" << "\n";
        targets[i & 0xFF].first  = argv[i];
        targets[i & 0xFF].second = "out-" + std::to_string(i) + ".bin";
    }
